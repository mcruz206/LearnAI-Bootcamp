...
1. We will now create a resource group to contain our Azure BatchAI jobs. Azure BatchAI is right now available in a limited number of regions. We will be using "East US". Type `az group create --name sethmottazbatch --location eastus` to create the resource group and type `az configure --defaults group=sethmottazbatch` and `az configure --defaults location=eastus` to make the resource group and location default for the current session.
2. We will also create a storage account for storing data and script. Type `az storage account create --name sethmottbatchstore --sku Standard_LRS` to do so.
...
```
set AZURE_STORAGE_ACCOUNT=sethmottbatchstore
az storage account keys list --account-name sethmottbatchstore -o tsv --query [0].value > deleteme.txt
set /p AZURE_STORAGE_KEY=< deleteme.txt
set AZURE_BATCHAI_STORAGE_ACCOUNT=sethmottbatchstore
...
```
export AZURE_STORAGE_ACCOUNT=sethmottbatchstore
export AZURE_STORAGE_KEY=$(az storage account keys list --account-name sethmottbatchstore -o tsv --query [0].value)
export AZURE_BATCHAI_STORAGE_ACCOUNT=sethmottbatchstore
export AZURE_BATCHAI_STORAGE_KEY=$(az storage account keys list --account-name sethmottbatchstore -o tsv --query [0].value)
```
...
```
az batchai cluster create --name mycluster --vm-size STANDARD_NC6 --image UbuntuLTS --min 1 --max 1 --afs-name batchaiquickstart --afs-mount-path azurefileshare --user-name sethmott --password DataScience2017!
```
Then type `az batchai cluster list -o table` to confirm that the cluster was created. Find the cluster on the Azure portal to make sure you can see it on the portal as well. 
...
```
5. To interrupt a job while it's running, simply type `CTRL+C` on the command line and confirm that you intend to do so (for this lab, just say no to avoid killing the job). Wait for the job to finish and run `az batchai job delete --name myjob` to delete it. [[Not exactly sure what it means to delete a job after its finished running.]]
